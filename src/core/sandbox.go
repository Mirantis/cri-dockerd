...
func (ds *dockerService) RunPodSandbox(
	...
	if containerConfig.GetLinux().GetSecurityContext().GetNamespaceOptions().GetNetwork() == runtimeapi.NamespaceMode_NODE {
		return resp, nil
	}
...
func (ds *dockerService) StopPodSandbox(
	ctx context.Context,
	r *runtimeapi.StopPodSandboxRequest,
	...
	if statusErr == nil {
		...
		if checkpointErr != nil {
			if checkpointErr != store.ErrCheckpointNotFound {
				...
				if err != nil {
					logrus.Error(err, "Failed to delete corrupt checkpoint for sandbox", "podSandboxID", podSandboxID)
				}
			}
			if libdocker.IsContainerNotFoundError(statusErr) {
				logrus.Info("Both sandbox container and checkpoint could not be found. Proceed without further sandbox information.", "podSandboxID", podSandboxID)
			} else {
	...
	if err := ds.client.StopContainer(podSandboxID, defaultSandboxGracePeriod); err != nil {
		...
		if !libdocker.IsContainerNotFoundError(err) {
			logrus.Error(err, "Failed to stop sandbox", "podSandboxID", podSandboxID)
			errList = append(errList, err)
		} else {
...
func (ds *dockerService) RemovePodSandbox(
	ctx context.Context,
	r *runtimeapi.RemovePodSandboxRequest,
	...
	for i := range containers {
		if _, err := ds.RemoveContainer(ctx, &runtimeapi.RemoveContainerRequest{ContainerId: containers[i].ID}); err != nil &&
...
func (ds *dockerService) ListPodSandbox(
	_ context.Context,
	r *runtimeapi.ListPodSandboxRequest,
